// src/components/SecureQRModal.jsx
// Enterprise-grade secure credential issuance
...
function SecureQRModal({ student, issuerInfo, onClose, onMarkAsIssued }) {
  ...
  useEffect(() => {
    generateClaimToken(); // Now it's async
  }, []);
  ...
  const generateClaimToken = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        console.error('❌ Failed to store token on backend');
      }
    } catch (error) {
